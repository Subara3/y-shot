...
1. **設定** (歯車ボタン): 対象URL、Basic認証、出力先、ヘッドレスモードを設定
...
テスト実行後、出力フォルダにタイムスタンプ付きサブフォルダが作成され、以下が生成されます:

- `*.png` — スクリーンショット画像
- `report.html` — スクショ一覧のHTMLレポート
- `evidence.xlsx` — スクショを縦に並べたExcelファイル（エビデンス用）

実行完了後に「出力フォルダを開く」ボタンからExplorerで直接開けます。
...
- max_length用の境界値パターン自動生成
- 要素ブラウザからのクイック追加（入力・クリック・選択）
- フォーム入力値の一括キャプチャ
- セレクタバンク（URL別に要素情報を保存）
- ヘッドレスモード（ブラウザ非表示で高速実行）
- Basic認証対応
- クリップボードコピー（Windowsのみ）
- 中断ボタン
